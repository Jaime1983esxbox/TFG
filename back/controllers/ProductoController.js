...
const agregar_imagen_galeria_admin = async function(req, res){
    if(req.user){
        if(req.user.role == 'Admin'){
            ...
            // Para agregar la imagen al array de galeria en un producto
...
const obtener_productos_id_publico = async function(req, res){
    var id = req.params['id'];

    let reg = await Producto.findOne({_id: id});
    res.status(200).send({data:reg});
}
