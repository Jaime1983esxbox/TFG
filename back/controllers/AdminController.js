...
const kpi_ganancias_mensuales_admin = async function(req, res){
    if (req.user) {
        if (req.user.role == 'Admin') {
            ...
            let currenrt_month = current_date.getMonth()+1;
            ...
            for(var item of reg){
                ...
                if(createdAt_date.getFullYear() == current_year){
                    ...
                    if(mes == currenrt_month){
                        total_month = total_month + item.subtotal;
                        count_ventas = count_ventas + 1;
                    }

                    if(mes == currenrt_month - 1){
                        total_previous_month = total_previous_month + item.subtotal;
                    }
